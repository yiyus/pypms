...
def miller(r):
    return r[:,2],r[:,0]
def variants(sym,r):
    return np.outer(sym,r)

# symmetry operators
cubic_ea=rad(np.transpose(np.array([
    [0,0,0],[90,0,0],[180,0,0],[270,0,0], # white
    [0,90,0],[90,90,0],[180,90,0],[270,90,0], # red
    [0,180,0],[90,180,0],[180,180,0],[270,180,0], # yellow
    [0,270,0],[90,270,0],[180,270,0],[270,270,0], # orange
    [90,90,0],[90,90,90],[90,90,180],[90,90,270], # blue
    [270,90,0],[270,90,90],[270,90,180],[270,90,270] # green
])))
cubic_Rphi1=Rz(cubic_ea[0])
cubic_RPhi=Rx(cubic_ea[1])
cubic_Rphi2=Rz(cubic_ea[2])
cubic=cubic_Rphi2@cubic_RPhi@cubic_Rphi1
# for c in np.unstack(cubic): print(rnd(c))
...
def print_cubic(r):
    c=variants(cubic,r)
    print("cubic:\n")
    for ci in np.unstack(c,axis=2): print(ci)
